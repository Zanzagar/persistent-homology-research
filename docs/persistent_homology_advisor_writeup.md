...
## Application: Design Choices, the Two-Pipeline Architecture, and the Big Picture

The practical question is: how does persistent homology actually serve the two-pipeline architecture that structures this research? Below I discuss the specific design choices, sketch how the pieces fit together, and outline the conceptual shift in how PH integrates with the broader framework.
...
### PH as Epistemic Anchor: The Conceptual Shift

The most important conceptual shift in this work is that persistent homology is not simply "one of three parallel feature extractors." It is the only pathway with a mathematical stability guarantee, and this asymmetry should govern the entire architecture. The three pathways produce features of fundamentally different epistemic status:
...
This asymmetry means the three pathways should not be treated symmetrically in fusion. PH features should serve as the anchor—the features we trust most—and the other pathways should be measured against them. The high-level idea is a confidence hierarchy with four tiers:

| Tier | Source | Epistemic Basis |
|------|--------|----------------|
| 1 — Proven | PH features | Stability theorem |
| 2 — Corroborated | Features all three pathways agree on | Cross-pathway convergence |
| 3 — Empirically invariant | Features passing LOGO but low alignment with PH | Empirical testing only |
| 4 — Uncertain | Unique to one pathway | No cross-validation |

The tools for populating this hierarchy include CKA (Centered Kernel Alignment; Kornblith et al., 2019) for measuring global alignment between pathways, and SLIDE decomposition (Gaynanova and Li, 2019) for identifying which specific feature dimensions are shared versus unique across pathways. Features higher in this hierarchy receive greater weight in retrieval.

### Pipeline A (Repository/Cataloguing)

Full-resolution analog images—generated by process-based simulators like Flumy (meandering) or BRAHMS (braided)—are encoded via all three pathways. The SEDT filtration is applied to each image, cubical persistence is computed in $H_0$ and $H_1$, and the resulting persistence diagrams are vectorized into 20–50-dimensional feature vectors. These are combined with classical geostatistical features (~50 dimensions) and learned features from DINOv2 (~768 dimensions).

The key architectural idea is that each analog's index entry becomes a *tiered descriptor* rather than a flat feature vector:

- **Core index** (Tiers 1–2): PH features and cross-pathway corroborated features. Primary retrieval keys—the features with the strongest invariance guarantees.
...
- **Uncertainty envelope** (Tier 4): Single-pathway features and intra-class variability estimates. Inform uncertainty quantification rather than primary retrieval.
...
A geoscientist's query consists of sparse observations: a few well logs, a partial seismic survey, scattered outcrop measurements. These are fundamentally different from the complete images in Pipeline A—they are incomplete, multimodal, and defined on different data structures (1D depth profiles vs. 2D images). A Neural Process encoder maps these sparse inputs to a distribution $(\mu, \sigma)$ in the same embedding space—not a point estimate, but a calibrated uncertainty region that degrades gracefully with observation density. More observations tighten the distribution; fewer yield wider bounds.

The confidence hierarchy provides a principled way to handle this asymmetry: dimensions corresponding to Tier 1 features get tight priors (PH constrains them strongly), while dimensions corresponding to Tier 4 features get diffuse priors (little a priori constraint). Features that are both well-constrained by observations *and* high-tier in the confidence hierarchy receive maximal weight in retrieval.

### The Compatibility Problem

This is the central architectural challenge: the essence representation computed from complete images (Pipeline A) must be compatible with the essence representation inferred from sparse observations (Pipeline B). The analog side has complete 2D images from which cubical persistence can be computed directly; the query side has sparse 1D profiles from which $H_1$ features cannot be computed directly—you cannot detect loops in a channel network from three vertical well intersections.
...
### Looking Forward: From Structural Similarity to Behavioral Similarity

Everything above addresses structural essence—does this analog look like the target in terms of spatial pattern? But the ultimate purpose of analog retrieval is to find analogs that *behave like* the target, not just look like it. If two geological configurations share the same topological structure, do they also produce similar dynamic responses to stimulation?

There is direct evidence suggesting yes: Moon et al. (2019) showed that PH features of pore structure predict macroscopic flow properties—topological invariants of static geometry carry information about dynamic behavior. This suggests a validation criterion for the entire system: analogs retrieved by Pipeline B should exhibit similar dynamic response when subjected to identical simulation protocols. If structural retrieval predicts response similarity, the system is validated at the deepest level. If not, the gap identifies which additional descriptors Pipeline A must incorporate.

The theoretical framework for this (Takens embedding of simulation time series, Morse-Smale decomposition of basins of attraction, and the equifinality caveat from Phillips, 2006) is developed in the dissertation chapter and represents a significant future direction.
