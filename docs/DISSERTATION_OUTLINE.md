...
**Li et al. 2025: sparse mixing and auxiliary-variation synergy.** Li, Fan, Zheng, et al. (2025, arXiv:2503.00639) strengthen the iVAE result by relaxing the exponential-family assumption and exploiting sparse structure in the mixing function $f$. Under their assumptions A1–A5—smooth positive density of $\theta$ conditional on generator identity (A1); conditional independence of $\theta$ components given the generator (A2); sufficient changes across domains (A3); sparse mixing structure constraining which latent components affect which image regions (A4); and a fully-connected fallback (A5)—two theorems follow:
...
**Why this fits the principle-based generator.** Four features of the QCF's experimental apparatus align with the Li et al. assumption structure:
...
2. **Mathematical invariance theorems**: Prove from first principles that a representation is stable under bounded perturbations (stability) or identifies true generative factors under auxiliary supervision (identifiability). The stability theorem (§3.6) provides the first for persistent homology; Li et al. (2025) Theorem 2 provides the second for CG-VAE identified factors (§5.8). Together, these form the two Level 1 theorem classes developed in §8.1 — stability (1a) and identifiability (1b) — each protecting against a distinct failure mode (§8.2). Mathematical proof is not defeasible by additional evidence; it does not depend on which generators are tested, how many are used, or how "diverse" the test pool is.
...
**Level 1b — Identifiability (correspondence with generative factors).** Identifiability theorems (Khemakhem et al., 2020; Li et al., 2025) provide a formal proof that learned latent factors recover true generative parameters up to well-defined invertible equivalence classes, under auxiliary-variable supervision. Khemakhem et al.'s iVAE result (§5.8) establishes affine-equivalence identifiability when $2n + 1$ distinct auxiliary values produce linearly independent natural-parameter vectors of an exponential-family conditional prior. Li et al.'s Theorem 2 strengthens this to component-wise identifiability—up to element-wise invertible transformation—under sparse-mixing structure A4 and a second-order derivative condition on the mixing Jacobian. Section 5.8 contains the theorem statements and their relationship to the CG-VAE architecture introduced there.
...
| Descriptor indexes generator-specific texture or artifact that happens to correlate with environment label in the training data | Level 1b (identifiability) | Li et al. Theorem 2 bounds learned factor away from non-generative artifact via auxiliary supervision |
...
The most important conceptual principle governing this architecture is that the three pathways are not parallel feature extractors of equal epistemic standing. Two theorem-backed classes anchor the architecture: PH features have a mathematical stability guarantee (bounded sensitivity to input perturbation; §3.6), and CG-VAE factors passing the pre-committed MCC threshold have a mathematical identifiability guarantee (correspondence with true generative factors up to invertible transformation; §5.8). Classical geostatistical features are well-understood but depend on stationarity assumptions that geological data routinely violate, and have no analogous theorem. DINOv2 residuals capture rich visual semantics but are opaque, entangled, and dependent on training distribution. This epistemic asymmetry means the pathways should not be treated symmetrically in fusion. The two theorem-backed classes serve as the *epistemic anchors*—the features around which the architecture is organized, with the doubly-Level-1 joint component (features both stable and identified) at the top of the confidence hierarchy (§14.6)—and other pathways are measured against them. Section 14.6 develops this principle into a formal confidence hierarchy with operationalized tiers; here, we show how it shapes the system architecture at every level.
...
Persistent homology and supervised-identifiable learned features (CG-VAE) occupy the privileged Level 1 positions within this architecture. The stability theorem (§3.6) guarantees that topological descriptors satisfy Level 1a of the evidence hierarchy; Li et al. (2025) Theorem 2 guarantees that CG-VAE factors passing the pre-committed MCC threshold satisfy Level 1b (§8.1). The two guarantees protect against different failure modes: stability bounds sensitivity to input perturbation, while identifiability bounds correspondence to true generative factors (§8.2). Descriptors satisfying both simultaneously—the doubly-Level-1 joint component isolated by SLIDE (§14.6)—are provably resistant to the generator-artifact contamination that would render the index scientifically indefensible. Classical features and DINOv2 residuals, while providing complementary discriminative power, enter the index at lower evidence levels and are accordingly weighted in the similarity computation.
...
| Interventional sufficiency ($2 \lvert z_a \rvert + 1$ informative changes per factor subset $z_a$) | Strong: element-wise identifiability up to invertible transformation | Li et al. (2025) Theorem 2; A1–A4 satisfied |
...
The MCC thresholds follow the Li et al. (2025) convention; they may be refined per experimental protocol but must be pre-committed before held-out evaluation.
...
| CG-VAE (identified) | $S_k^{(\textrm{CG-VAE})} \geq 2$ for factors in $z_a$ per A4 | Matches Li et al. (2025) Theorem 2 expectations |
...
| 1b — Proven (identifiability) | CG-VAE factors passing MCC $\geq 0.85$ | Li et al. (2025) Theorem 2 | $\hat{z}$ identifies true $z$ up to element-wise invertible transformation |
...
- **PH features**: Tier 1a anchor — bounded invariants via the stability theorem
- **CG-VAE identified factors** (passing MCC threshold): Tier 1b anchor — correspondence with true generative factors via Li et al. (2025) Theorem 2; features in the PH $\cap$ CG-VAE joint component occupy the doubly-Level-1 Tier 1-joint (§14.6)
...
The framework's central epistemological contribution is the recognition that the three pathways provide evidence of fundamentally *different* epistemic status—and that this asymmetry should govern the entire architecture. Two theorem-backed classes anchor the architecture: persistent homology via the stability theorem (Level 1a, bounded sensitivity to perturbation) and supervised-identifiable learned features (CG-VAE) via Li et al. (2025) Theorem 2 (Level 1b, correspondence with true generative factors). The two guarantees are complementary, protecting against distinct failure modes; their joint component is the doubly-Level-1 class at the top of the confidence hierarchy. Classical geostatistical descriptors provide well-understood spatial characterization grounded in decades of established theory. Self-supervised visual representations (DINOv2) capture the holistic pattern recognition that experienced geoscientists perform intuitively. The framework's power comes not from any single pathway but from their *convergence*: when topology, statistics, and learned semantics agree on a structural characterization—and when that agreement sits on top of the two-Level-1 theorem foundation—the evidence for essence is strongest.

The confidence hierarchy of Section 14.6 operationalizes this principle: PH features and CG-VAE identified factors anchor the architecture as Tier 1a and Tier 1b descriptors respectively, with features in their joint component at Tier 1-joint (doubly Level 1); cross-pathway agreement at additional pathways provides Tier 2 corroboration; and the epistemic asymmetry between theorem-backed and empirically derived features—previously unexploited in multi-modal fusion research—structures how evidence from different pathways is weighted, combined, and trusted.
...
Brady, J., von Kügelgen, J., Lachapelle, S., Buchholz, S., Kipf, T., & Brendel, W. (2025). Interaction asymmetry: A general principle for learning composable abstractions. *International Conference on Learning Representations (ICLR 2025)*. *arXiv:2411.07784*.

Brehmer, J., et al. (2022). Weakly supervised causal representation learning. *Advances in Neural Information Processing Systems (NeurIPS) 35*.
...
Khemakhem, I., Kingma, D. P., Monti, R. P., & Hyvärinen, A. (2020). Variational autoencoders and nonlinear ICA: A unifying framework. *Proceedings of the Twenty-Third International Conference on Artificial Intelligence and Statistics (AISTATS)*.
...
Li, Z., Fan, S., Zheng, Y., Ng, I., Xie, S., Chen, G., Dong, X., Cai, R., & Zhang, K. (2025). Synergy between sufficient changes and sparse mixing procedure for disentangled representation learning. *arXiv:2503.00639*.
...
Locatello, F., Bauer, S., Lučić, M., Raetsch, G., Gelly, S., Schölkopf, B., & Bachem, O. (2019). Challenging common assumptions in the unsupervised learning of disentangled representations. *Proceedings of the 36th International Conference on Machine Learning (ICML)*, 4114–4124.
...
Rongier, G., & Peeters, L. (2025). Towards geological inference with process-based and deep generative modeling, part 2: Inversion of fluvial deposits and latent-space disentanglement. *arXiv:2510.17478*.
...
Squires, C., et al. (2023). Linear causal disentanglement via interventions. *Proceedings of the 40th International Conference on Machine Learning (ICML)*.
...
Varici, B., et al. (2024). Score-based causal representation learning. *Journal of Machine Learning Research*.
...
von Kügelgen, J., Sharma, Y., Gresele, L., Brendel, W., Schölkopf, B., Besserve, M., & Locatello, F. (2021). Self-supervised learning with data augmentations provably isolates content from style. *Advances in Neural Information Processing Systems (NeurIPS) 34*.
...
Yao, D., Rancati, D., Cadei, R., Fumero, M., & Locatello, F. (2025). Unifying causal representation learning with the invariance principle. *International Conference on Learning Representations (ICLR 2025)*. *arXiv:2409.02772*.
