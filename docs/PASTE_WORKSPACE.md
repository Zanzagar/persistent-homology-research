...
Two design choices require justification before we discuss the architecture. First, we use cubical complexes rather than Vietoris-Rips complexes. Our input data are regular pixel grids, not scattered point clouds, and cubical complexes respect this grid structure natively — each pixel is treated as an elementary square (a 2-cube), and the filtration is built by thresholding pixel values. This avoids the quadratic memory cost of constructing a Vietoris-Rips complex from all pairwise distances, which would be prohibitive for $256 \times 256$ images with 65,536 points. The implementation uses Giotto-TDA, which handles cubical complexes efficiently. Second, we use the signed Euclidean distance transform (SEDT) as our filtration function. The SEDT assigns each pixel a signed distance to the nearest facies boundary — positive inside channel bodies, negative inside floodplain. This choice is geologically motivated: filtering by increasing SEDT values progressively erodes narrow connections, revealing which topological features are robust at the core of geobodies versus those that depend on thin, possibly artifactual connections. A loop that persists across a wide range of SEDT values corresponds to a thick, well-connected braided channel network. A loop that dies quickly corresponds to a thin connection that may be an artifact of grid resolution or generator noise. The filtration choice is doing real epistemic work here: it translates the mathematical abstraction of persistence into a geologically interpretable quantity — the physical thickness of the channels that form the loop.

With these design choices established, I want to communicate the most important conceptual shift in my thinking since beginning this work: persistent homology is not simply "one of three parallel feature extractors." It is the only pathway with a mathematical stability guarantee, and this asymmetry should govern the entire architecture. The three pathways produce features of fundamentally different epistemic status. PH features are backed by the stability theorem — a mathematical proof of bounded sensitivity to input perturbation. Classical geostatistical features (variograms, MPS statistics) are well-understood but depend on stationarity assumptions that geological data routinely violates, and have no analogous stability theorem. DINOv2 learned features capture rich visual semantics but are opaque, entangled, and dependent on the training distribution. This asymmetry means we should not treat the three pathways symmetrically in fusion. PH features should serve as the epistemic anchor — the features we trust most — and the other pathways should be measured against them. I am still working out the precise mechanisms for this, but the high-level idea is a confidence hierarchy: at the top sit PH features themselves (Tier 1), backed by a mathematical theorem; next are features that all three pathways converge on (Tier 2), where cross-pathway agreement provides strong corroboration; then features that pass empirical invariance tests like LOGO but show low alignment with PH (Tier 3), meaning they are empirically robust but lack theoretical backing; and finally features unique to a single pathway (Tier 4), which are potentially meaningful but also potentially artifactual. The tools for populating this hierarchy include CKA (Centered Kernel Alignment) for measuring global alignment between pathways and SLIDE decomposition for identifying which specific feature dimensions are shared versus unique. I want to discuss the details of this with Dr. Srinivasan at our next meeting, but the principle is that features higher in this hierarchy should receive greater weight in retrieval.

This confidence hierarchy has concrete consequences for how the two pipelines are designed. In Pipeline A (the repository and cataloguing side), full-resolution analog images — generated by process-based simulators like Flumy (meandering) or BRAHMS (braided) — are encoded via all three pathways. The SEDT filtration is applied to each image, cubical persistence is computed in $H_0$ and $H_1$, and the resulting persistence diagrams are vectorized into 20–50-dimensional feature vectors. These are combined with classical geostatistical features (~50 dimensions from variogram parameters and MPS template frequencies) and learned features from DINOv2 (~768 dimensions). The key architectural idea is that each analog's index entry becomes a tiered descriptor rather than a flat feature vector: a core index built from Tier 1–2 features serves as the primary retrieval key, an extended index of Tier 3 features provides refinement within equivalence classes, and an uncertainty envelope from Tier 4 features informs uncertainty quantification rather than primary retrieval. This means Pipeline A does not merely store analogs — it stores analogs with calibrated confidence about what is known and unknown about each one.

In Pipeline B (the query and retrieval side), a geoscientist's query consists of sparse observations: a few well logs, a partial seismic survey, scattered outcrop measurements. These are fundamentally different from the complete images in Pipeline A — they are incomplete, multimodal, and defined on different data structures (1D depth profiles versus 2D images). A Neural Process encoder maps these sparse inputs to a distribution $(\mu, \sigma)$ in the same embedding space — not a point estimate, but a calibrated uncertainty region that degrades gracefully with observation density. More observations tighten the distribution; fewer yield wider uncertainty bounds. The confidence hierarchy provides a principled way to handle this asymmetry: dimensions corresponding to Tier 1 features get tight priors because PH constrains them strongly, while dimensions corresponding to Tier 4 features get diffuse priors reflecting the lack of a priori constraint.

The central architectural challenge binding these two pipelines together is the compatibility problem, and it is worth stating plainly: the essence representation computed from complete images in Pipeline A must be compatible with the essence representation inferred from sparse observations in Pipeline B. These two sides face an information asymmetry — the analog side has complete 2D images from which cubical persistence can be computed directly, but the query side has sparse 1D profiles from which $H_1$ features cannot be computed directly. You simply cannot detect loops in a channel network from three vertical well intersections. Persistent homology contributes to resolving this in two ways. First, the stability theorem provides a framework for reasoning about how much information about the persistence diagram can be recovered from partial observations — if the sparse data constrains the underlying function $f$ within an $\epsilon$-band, then the persistence diagram is constrained within an $\epsilon$-band as well. Second, the convergence regularization loss across all three pathways enforces that the topological, geostatistical, and learned representations must agree during training — so even if the sparse query encoder cannot directly compute $H_1$, the convergence loss ensures that the learned representation (which can be computed from sparse inputs) captures information correlated with the topological features.

I want to flag one extension that I think significantly broadens the ambition of this framework, though it is still in early stages. Everything above addresses structural essence — does this analog look like the target reservoir in terms of its spatial pattern? But the ultimate purpose of analog retrieval is to find analogs that behave like the target, not just look like it. If two geological configurations share the same topological structure ($H_1$ features), do they also produce similar dynamic responses to stimulation? There is direct evidence suggesting yes: Moon et al. (2019) showed that PH features of pore structure predict macroscopic flow properties — topological invariants of static geometry carry information about dynamic behavior. This suggests a validation criterion for the entire retrieval system: analogs retrieved by Pipeline B based on structural similarity should exhibit similar dynamic response when subjected to identical simulation protocols. If structural retrieval predicts response similarity, the system is validated at the deepest level. If not, the gap between structural and response essence tells us which additional descriptors Pipeline A needs to incorporate. I am still developing the theoretical framework for this — it involves Takens embedding of simulation time series and Morse-Smale decomposition of basins of attraction — and I want to discuss it with Dr. Srinivasan as a potential future direction.

I want to be honest about what this framework does not yet fully answer. Whether the $H_1$ hypothesis holds at the predicted accuracy level is an empirical question that the next phase of experiments will address. Whether the convergence regularization successfully transfers topological information to the sparse-data encoder is testable but untested. The confidence hierarchy and its mechanisms — CKA, SLIDE decomposition, Dempster-Shafer Theory for asymmetric evidence fusion — are theoretically motivated but need implementation and validation. And the response essence extension is the most speculative piece, requiring us to link topological invariants of geological structure to dynamical systems properties of reservoir response, which is a significant theoretical and computational challenge. The mathematical foundations are solid; the experimental validation is the next frontier. I look forward to discussing which of these directions to prioritize in our next meeting.
